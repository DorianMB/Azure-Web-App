...
const _generateParametersMetadataForGroup = (group, solution, config) => {
  ...
  for (const parameterId of parametersIds) {
    ...
    if (parameterData !== undefined) {
      parametersMetadata.push(parameterData);
    }
  }
...
// file.
const generateParametersMetadata = (solution, config, runTemplateParametersIds) => {
  const parametersMetadataList = runTemplateParametersIds.map(parameterId => _generateParameterMetadata(
    parameterId, solution, config));
